...
class ShoppingCart(models.Model):
    ...
    def get_cart_total(self):
        return sum([item.item.discounted_price*item.quantity for item in self.orderitems.filter(is_ordered = False)])
    
    def get_cart_coupon(self):
        sum = 0
        for item in self.orderitems.filter(is_ordered = False):
            if item.is_coupon:
                sum += (item.item.discounted_price - item.item.discounted_price*item.item.vendor.profile_vendor.discount_on_coupon_code/100)*item.quantity
            else:
