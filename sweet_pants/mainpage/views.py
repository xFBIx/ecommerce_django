...
class ProductListView(PurposeRequiredMixin, ListView):
    ...
    def get_context_data(self,*args, **kwargs):
        ...
        userr_list = Product.objects.all().filter( vendor = self.request.user.id).order_by('-sales')
...
class ProductDetailView(PurposeRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def post(self, request, pk, **kwargs):
        ...
        if not request.user.shoppingcart.orderitems.filter(item=product, is_ordered=False):
            if quantity > product.quantity:
                messages.warning(request, f"{quantity} quantities of this product is not available please select lesser")
                return redirect('product-detail', pk=pk)
            item = Items.objects.create(item=product, quantity=quantity, customer=request.user)
...
class ProductCreateView(VendorRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        form.instance.discounted_price = form.instance.price - form.instance.price*form.instance.discount/100
...
def remove_shoppingcart(request, pk):
    product = Product.objects.filter(id=pk).first()
    item_to_delete = request.user.shoppingcart.orderitems.filter(item=product, is_ordered=False)
...
def checkout(request):
    context = {'address': request.user.profile_customer.address , 'no_address' : 'Please add an address before placing order!', 
               'balance': request.user.profile_customer.balance, 'items' : request.user.shoppingcart.orderitems.all().filter(is_ordered=False),
    ...
    if request.method == 'POST':
        ...
        if request.user.shoppingcart.orderitems.filter(item=product, is_ordered=False):
            if quantity > product.quantity:
                messages.warning(request, f"{quantity} quantities of this product is not available please select lesser")
                return redirect('shoppingcart')
            data = request.user.shoppingcart.orderitems.filter(item=product, is_ordered=False).first()
...
def mail(vendor_email,vendor,customer,quantity,product,amount):
    ...
    data = {'Messages': [{"From": {"Email": "rudradattdave@gmail.com","Name": "Sweet Pants"},
...
def buynow(request):
    if request.user.shoppingcart.orderitems.all().filter(is_ordered=False):
        if not request.user.profile_customer.address:
            messages.warning(request, "Please add an dilevery address first")
            return redirect('checkout')
        for item in request.user.shoppingcart.orderitems.filter(is_ordered = False):
            if item.quantity > item.item.quantity:
                messages.warning(request, f"{item.quantity} quantities of this product is not available please select lesser")
                return redirect('checkout')
        if request.user.profile_customer.balance < request.user.shoppingcart.get_cart_coupon():
            messages.warning(request, "Insufficient account balance, Please add balance")
            return redirect('checkout')
        request.user.profile_customer.balance -= request.user.shoppingcart.get_cart_coupon()
        ...
        for item in request.user.shoppingcart.orderitems.filter(is_ordered = False):
            ...
            if item.is_coupon:
                item.orderprice = (item.item.discounted_price - item.item.discounted_price*item.item.vendor.profile_vendor.discount_on_coupon_code/100)*item.quantity
            else:
                item.orderprice = item.item.discounted_price*item.quantity
            item.is_ordered = True 
            item.save()
            mail(item.item.vendor.email,item.item.vendor.username,request.user.username,item.quantity,item.item.title,item.orderprice)
...
def review(request, pk):
    ...
    if request.user.shoppingcart.orderitems.filter(item=product,is_ordered = True):
        if request.method == 'POST':
            ...
            Review.objects.create(product=product, description=review, date=datetime.now(), customer=request.user)
