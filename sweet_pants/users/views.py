...
from .forms import ProfileUserUpdateForm, ProfileLibrarianUpdateForm, purposeform
...
def profile(request):
    if request.user.groups.all()[0].name == "User":
        ...
        context = {
            "coupon": request.user.profile_vendor.coupon,
            "no_coupon": "No Coupon Code!",
            "dis_coup": request.user.profile_vendor.discount_on_coupon_code,
        }
    ...
    if request.method == "POST":
        if request.user.groups.all()[0].name == "User":
            form = ProfileUserUpdateForm(
                request.POST, request.FILES, instance=request.user.profile_customer
            )
        elif request.user.groups.all()[0].name == "Librarian":
            form = ProfileLibrarianUpdateForm(
                request.POST, request.FILES, instance=request.user.profile_vendor
            )
        if form.is_valid():
            form.save()
        ...
        if request.user.groups.all()[0].name == "User":
            form = ProfileUserUpdateForm(instance=request.user.profile_customer)
        elif request.user.groups.all()[0].name == "Librarian":
            form = ProfileLibrarianUpdateForm(instance=request.user.profile_vendor)
